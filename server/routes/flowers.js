...
router.post("/search", (_req, _res) => {
    ...
    if (!name && !meaning) {
        return _res.status(400).send("Please provide either a name or a meaning to search."); // Return a 400 status with a message
    }
    ...
    const matchingFlowers = flowersDataJSON.filter((data) => {
        ...
        const meaningMatches = meaningRegex && data.meaning.some((m) => meaningRegex.test(m)); // Check by meaning using regex
